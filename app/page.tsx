...
const ageFor = (item: Watch) => { const date = item.listedOn || item.boughtOn; const timestamp = new Date(`${date}T12:00:00`).getTime(); return Number.isFinite(timestamp) ? Math.max(0, Math.floor((Date.now() - timestamp) / 86400000)) : 0; };
...
export default function Home() {
  ...
  const visibleBids = useMemo(() => bids.filter((item) => `${item.model} ${item.name} ${item.category || ""}`.toLowerCase().includes(bidSearch.toLowerCase())).sort((a, b) => b.updatedOn.localeCompare(a.updatedOn)), [bids, bidSearch]);
  ...
  return <main className="app-shell">
    <header className="app-topbar">
      ...
      <div className="top-actions"><input className="global-search" aria-label="Search inventory" placeholder="Search inventory" value={search} onChange={(event) => changeSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") setView("inventory"); }} /><button className="text-button backup-button" onClick={backup}>Back up</button><label className="text-button backup-button">Restore<input type="file" accept="application/json" onChange={restore} /></label><button className="primary-button" onClick={() => setShowForm(true)}>+ Add item</button></div>
    </header>
    ...
    {view === "bids" && <BidGuide entries={visibleBids} watches={watches} search={bidSearch} setSearch={setBidSearch} add={() => setShowBidForm(true)} remove={removeBid} />}
...
function BidGuide({ entries, watches, search, setSearch, add, remove }: { entries: BidGuideEntry[]; watches: Watch[]; search: string; setSearch: (value: string) => void; add: () => void; remove: (id: string) => void }) {
  return <section className="page-view"><div className="page-heading"><div><p className="eyebrow">WHATNOT SOURCING</p><h1>Bid guide</h1><p>Save your ceiling once so you can bid quickly and stay disciplined.</p></div><button className="primary-button" onClick={add}>+ Add max bid</button></div><div className="bid-search-row"><input aria-label="Search bid guide" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search model or item name" /><span>Max bids are all-in ceilings.</span></div><div className="bid-grid">{entries.map((entry) => { const prior = watches.filter((item) => item.model.trim().toLowerCase() === entry.model.trim().toLowerCase()); const costs = prior.map(totalCost); return <article className="bid-card" key={entry.id}><div className="bid-card-top"><div><span className="bid-model">{entry.model}</span><h2>{entry.name || "Untitled item"}</h2><small>{entry.category || "Other"}</small></div><button aria-label={`Remove ${entry.model} from bid guide`} onClick={() => remove(entry.id)}>×</button></div><div className="bid-price"><span>Max all-in bid</span><strong>{money(entry.maxBid)}</strong></div>{entry.expectedSale ? <div className="bid-detail"><span>Expected sale</span><b>{money(entry.expectedSale)}</b></div> : null}{prior.length ? <div className="bid-detail"><span>Your past buys</span><b>{prior.length} · {money(Math.min(...costs))}–{money(Math.max(...costs))}</b></div> : null}{entry.notes ? <p>{entry.notes}</p> : <p className="muted">Add notes like color, sell price, or why this is a pass.</p>}<small className="bid-date">Updated {new Date(`${entry.updatedOn}T12:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</small></article>; })}{!entries.length && <div className="empty bid-empty"><b>No saved bids yet.</b><span>When a model comes up often, save its maximum all-in price here.</span></div>}</div></section>;
}
...
function BidGuideForm({ onClose, onSave }: { onClose: () => void; onSave: (entry: BidGuideEntry) => void }) { const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); const model = String(form.get("model") || "").trim(); const maxBidField = form.get("maxBid"); const maxBid = Number(maxBidField); if (!model || maxBidField == null || maxBidField === "" || !Number.isFinite(maxBid)) return; onSave({ id: crypto.randomUUID(), model, name: String(form.get("name") || "").trim(), category: String(form.get("category") || "Other").trim(), maxBid: round2(maxBid), expectedSale: form.get("expectedSale") ? round2(Number(form.get("expectedSale"))) : undefined, notes: String(form.get("notes") || "").trim(), updatedOn: new Date().toISOString().slice(0, 10) }); }; return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Add bid guide item"><form className="modal bid-form" onSubmit={submit}><button type="button" className="close" onClick={onClose}>×</button><p className="eyebrow">WHATNOT BID GUIDE</p><h2>Save a max bid</h2><p className="modal-copy">Use the total you are comfortable paying, including shipping and tax if those apply. Give variants different names to save separate ceilings.</p><div className="form-grid"><label>Model number<input name="model" autoFocus placeholder="e.g. 70271" required /></label><label>Item name / variant<input name="name" placeholder="e.g. Invicta Celestial, pink dial" /></label><label>Category<input name="category" placeholder="e.g. Watches, Sunglasses" /></label><label>Max all-in bid<input name="maxBid" type="number" min="0" step="0.01" placeholder="0.00" required /></label><label>Expected sale price<input name="expectedSale" type="number" min="0" step="0.01" placeholder="Optional" /></label><label className="full">Notes<input name="notes" placeholder="Why it is a good buy, colors to prefer, expected profit…" /></label></div><div className="modal-actions"><button type="button" className="text-button" onClick={onClose}>Cancel</button><button className="primary-button" type="submit">Save max bid</button></div></form></div>; }
