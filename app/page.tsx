...
type Watch = { id: string; model: string; name: string; category: string; boughtOn: string; cost: number; inbound: number; listPrice: number; status: Status; paidBy?: "Caroline" | "Ben"; soldPrice?: number; soldOn?: string; listedOn?: string; poshEarnings?: number; tracking?: string; poshUrl?: string; notes?: string };
...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {view === "cash" && <CashView stats={stats} ledger={ledger} watches={watches} />}
...
function SalesView({ stats, monthly }: { stats: { sold: Watch[]; profit: number; earnings: number; soldCost: number; roi: number }; monthly: { label: string; payout: number; fees: number; cost: number; profit: number; sold: number }[] }) {
  ...
  const estimatedBuyerCount = stats.sold.filter((item) => item.soldPrice == null && item.poshEarnings != null).length;
  const estimatedFeeCount = stats.sold.filter((item) => item.soldPrice == null || item.poshEarnings == null).length;
  const singleMonth = monthly[0];
  const benSold = stats.sold.filter((item) => item.paidBy === "Ben");
  const benPayout = round2(benSold.reduce((sum, item) => sum + earningsFor(item), 0));
  const benProfit = round2(benSold.reduce((sum, item) => sum + profitFor(item), 0));
  return <section className="page-view"><div className="page-heading"><div><p className="eyebrow">SALES &amp; PROFIT</p><h1>What you made</h1><p>Clear monthly results after Poshmark fees and item costs.</p></div></div><div className="report-grid"><article className="report-card chart-card"><div className="section-heading"><div><h2>Profit by month</h2><p>Actual profit—not listed value.</p></div></div>{monthly.length === 1 && singleMonth ? <div className="single-month-profit"><div><span>{singleMonth.label}</span><b className={singleMonth.profit >= 0 ? "profit" : "loss"}>{singleMonth.profit >= 0 ? "+" : ""}{money(singleMonth.profit)}</b><small>{singleMonth.sold} sold · {money(singleMonth.profit / singleMonth.sold)} average profit per sale</small></div><div className="single-month-stats"><span>Buyer paid <b>{money(grossSales)}</b></span><span>Poshmark payout <b>{money(singleMonth.payout)}</b></span></div></div> : monthly.length ? <div className="profit-chart">{monthly.map((month) => <div className="bar-group" key={month.label}><b className={month.profit < 0 ? "loss" : ""}>{money(month.profit)}</b><div className="bar-track"><div className={month.profit < 0 ? "bar loss-bar" : "bar"} style={{ height: `${Math.max(4, (Math.abs(month.profit) / maxMagnitude) * 50)}%` }} /></div><span>{month.label}</span></div>)}</div> : <div className="empty">Your completed sales will appear here.</div>}</article><article className="report-card"><h2>All-time sales</h2><div className="money-breakdown"><div><span>Buyer paid{estimatedBuyerCount ? ` (estimated on ${estimatedBuyerCount})` : ""}</span><b>{money(grossSales)}</b></div><div><span>Poshmark took{estimatedFeeCount ? ` (estimated on ${estimatedFeeCount})` : ""}</span><b>{money(totalFees)}</b></div><div><span>Item cost</span><b>{money(stats.soldCost)}</b></div><div className="net"><span>Actual profit</span><b>{money(stats.profit)}</b></div>{benSold.length > 0 && <div><span>Ben&apos;s {benSold.length} sold item{benSold.length === 1 ? "" : "s"}</span><b>{money(benPayout)} payout · {money(benProfit)} profit</b></div>}</div></article></div><div className="monthly-results"><div className="section-heading"><div><h2>Monthly detail</h2><p>Every month stays separate, so growth is easy to see.</p></div></div>{monthly.map((month) => <article className="monthly-result" key={month.label}><b>{month.label}</b><span>{month.sold} sold</span><span>Payout {money(month.payout)}</span><strong className={month.profit >= 0 ? "profit" : "loss"}>{money(month.profit)} profit</strong></article>)}{!monthly.length && <div className="empty">No completed sales yet.</div>}</div></section>;
}

function CashView({ stats, ledger, watches }: { stats: { investment: number; earnings: number; netCashInvested: number }; ledger: { id: string; date: string; type: "in" | "out"; label: string; amount: number }[]; watches: Watch[] }) {
  const people = (["Caroline", "Ben"] as const).map((person) => { const items = watches.filter((item) => (item.paidBy || "Caroline") === person); const putIn = round2(items.reduce((sum, item) => sum + totalCost(item), 0)); const payouts = round2(items.filter((item) => item.status === "Sold").reduce((sum, item) => sum + earningsFor(item), 0)); return { person, items: items.length, putIn, payouts, tiedUp: round2(putIn - payouts) }; });
  return <section className="page-view"><div className="page-heading"><div><p className="eyebrow">CASH FLOW</p><h1>Money in and out</h1><p>Sales payouts and purchases are different from profit—both matter.</p></div></div><div className="cash-person-grid">{people.map((person) => <article className="cash-person-card" key={person.person}><div><span>{person.person}</span><b>{money(person.putIn)}</b><small>put into {person.items} item{person.items === 1 ? "" : "s"}</small></div><dl><div><dt>Poshmark payouts</dt><dd>{money(person.payouts)}</dd></div><div><dt>Still tied up</dt><dd>{money(person.tiedUp)}</dd></div></dl></article>)}</div><section className="ledger-section"><div className="section-heading"><div><h2>Recent money movement</h2><p>Every purchase and Poshmark payout in date order.</p></div></div><div className="ledger">{ledger.map((entry) => <article key={entry.id}><div><b>{entry.label}</b><small>{new Date(`${entry.date}T12:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</small></div><strong className={entry.type === "in" ? "inflow" : "outflow"}>{entry.type === "in" ? "+" : "−"}{money(entry.amount)}</strong></article>)}</div></section></section>;
}
